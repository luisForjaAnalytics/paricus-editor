...
function useFileUpload(options) {
  ...
  const uploadFile = async file => {
    if (file.size > options.maxSize) {
      const error = new Error(i18next.t("errors.fileTooLarge", { size: options.maxSize / 1024 / 1024 }))
    ...
    try {
      if (!options.upload) {
        throw new Error(i18next.t("errors.uploadNotDefined"))
      }
      ...
      if (!url) throw new Error(i18next.t("errors.uploadFailed"))
        ...
        setFileItems((prev) =>
          prev.map((item) =>
            item.id === fileId
              ...
              : item))
        options.onError?.(error instanceof Error ? error : new Error(i18next.t("errors.uploadFailed")))
      }
      return null
    }
  }
  ...
  const uploadFiles = async files => {
    if (!files || files.length === 0) {
      options.onError?.(new Error(i18next.t("errors.noFiles")))
      return []
    }

    if (options.limit && files.length > options.limit) {
      options.onError?.(
        new Error(i18next.t("errors.maxFilesAllowed", { limit: options.limit }))
      )
      return []
    }
  ...
  return (
    <>
      ...
      <div className="tiptap-image-upload-content">
        <span className="tiptap-image-upload-text">
          <em>{t("upload.clickToUpload")}</em> {t("upload.dragAndDrop")}
        </span>
        <span className="tiptap-image-upload-subtext">
          {t("upload.maxFiles", { limit, size: maxSize / 1024 / 1024 })}
        </span>
      </div>
    </>
  )
...
export const ImageUploadNode = (props) => {
  ...
  const handleChange = (e) => {
    ...
    if (!files || files.length === 0) {
      extension.options.onError?.(new Error(i18next.t("errors.noFileSelected")))
      return
    }
        ...
        <div className="tiptap-image-upload-previews">
          {fileItems.length > 1 && (
            <div className="tiptap-image-upload-header">
              <span>{t("upload.uploading", { count: fileItems.length })}</span>
              <Button
                type="button"
                variant="ghost"
                onClick={(e) => {
